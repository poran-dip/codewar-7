...
export default function useSceneScroll(
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'ArrowUp' || e.key === 'PageUp' || e.key === 'w' || e.key === 'W') direction = 'up'
      else if (e.key === 'ArrowDown' || e.key === 'PageDown' || e.key === 's' || e.key === 'S') direction = 'down'
      else if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'A') direction = 'left'
      else if (e.key === 'ArrowRight' || e.key === 'd' || e.key === 'D') direction = 'right'
      else if (e.key === 'Enter' || e.key === 'f' || e.key === 'F') direction = 'enter'
      else if (e.key === 'Escape') direction = 'exit'
      else if (e.key === '1') { e.preventDefault(); lock(); onDirection('nav1'); return }
