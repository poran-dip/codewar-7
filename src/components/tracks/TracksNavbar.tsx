...
export default function TracksNavbar() {
  ...
  const codestellationNavItems = [
    ...
    {
      label: "PS",
      href: `/tracks/codestellation/ps`,
      key: "[2]",
      icon: FileCode,
    },
    {
      label: "RULES",
      href: `/tracks/codestellation/rules`,
      key: "[3]",
      icon: ScrollText,
    },
    {
      label: "PRIZES",
      href: `/tracks/codestellation/prizes`,
      key: "[4]",
      icon: Trophy,
    },
    {
      label: "FAQ",
      href: `/tracks/codestellation/faq`,
      key: "[5]",
      icon: HelpCircle,
    },
  ];
  ...
  const decodeNavItems = [
    ...
    {
      label: "RULES",
      href: `/tracks/decode/rules`,
      key: "[2]",
      icon: ScrollText,
    },
    {
      label: "PRIZES",
      href: `/tracks/decode/prizes`,
      key: "[3]",
      icon: Trophy,
    },
    ...
    >
      <div className="flex items-center gap-2 md:gap-3">
        ...
        >
          {/* Corner decorations */}
          <div
            className={`absolute -top-0.5 -left-0.5 w-2 h-2 md:w-3 md:h-3 border-t-2 border-l-2 transition-colors ${accentColor === "purple" ? "border-purple-400" : "border-cyan-400"}`}
          />
          <div
            className={`absolute -top-0.5 -right-0.5 w-2 h-2 md:w-3 md:h-3 border-t-2 border-r-2 transition-colors ${accentColor === "purple" ? "border-purple-400" : "border-cyan-400"}`}
          />
          <div
            className={`absolute -bottom-0.5 -left-0.5 w-2 h-2 md:w-3 md:h-3 border-b-2 border-l-2 transition-colors ${accentColor === "purple" ? "border-purple-400" : "border-cyan-400"}`}
          />
          <div
            className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 md:w-3 md:h-3 border-b-2 border-r-2 transition-colors ${accentColor === "purple" ? "border-purple-400" : "border-cyan-400"}`}
          />
                    ...
                    >
                      <div
                        ...
                        ${
                          isActive(item.href)
                            ...
                            : "bg-gray-900/50 text-gray-400 group-hover:text-" +
                              accentColor +
                              "-400 group-hover:bg-" +
                              accentColor +
                              "-500/10"
                      ...
                      >
                        ...
                        <span className="relative flex items-center gap-1.5 md:gap-2">
                          ...
                          {item.label}
                          <span
                            className={`hidden md:block text-[8px] md:text-[10px] ${accentColor === "purple" ? "text-purple-500/60" : "text-cyan-500/60"}`}
                          >
                            {item.key}
                          </span>
                        </span>
