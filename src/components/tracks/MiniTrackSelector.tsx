  ...
  return (
    <>
      ...
      >
        ...
        {tracks.map((track, i) => {
          ...
          return (
            ...
            >
              {/* Corner decorations */}
              <div
                className={`absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute top-0 right-0 w-3 h-3 border-t-2 border-r-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute bottom-0 left-0 w-3 h-3 border-b-2 border-l-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />

              {/* Scanline effect */}
              <div
                className={`absolute inset-0 bg-linear-to-b from-transparent to-transparent pointer-events-none ${colors.scanline}`}
              />
        ...
        {/* System label */}
        <div
          className={`mt-2 text-center text-[8px] font-mono tracking-widest ${
            activeTrack?.color === "purple"
              ? "text-purple-500/40"
              : "text-cyan-500/40"
          }`}
        >
      ...
      >
        ...
        {mobileTracks.map((track, i) => {
          ...
          return (
            ...
            >
              {/* Corner decorations */}
              <div
                className={`absolute top-0 left-0 w-2 h-2 border-t-2 border-l-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute top-0 right-0 w-2 h-2 border-t-2 border-r-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute bottom-0 left-0 w-2 h-2 border-b-2 border-l-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />
              <div
                className={`absolute bottom-0 right-0 w-2 h-2 border-b-2 border-r-2 transition-colors ${isActive || isHovered ? colors.corner : colors.border}`}
              />

              {/* Scanline effect */}
              <div
                className={`absolute inset-0 bg-linear-to-r from-transparent to-transparent pointer-events-none ${colors.scanline}`}
              />
        ...
        {/* System label */}
        <div
          className={`absolute -bottom-3.5 md:-bottom-5 left-1/2 -translate-x-1/2 text-[7px] font-mono tracking-widest whitespace-nowrap ${
            activeTrack?.color === "purple"
              ? "text-purple-500/40"
              : "text-cyan-500/40"
          }`}
