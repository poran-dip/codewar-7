          ...
          >
              ...
              {items.map((item, i) => (
                ...
                >
                  ...
                  >
                    {/* Corner decorations */}
                    <div
                      className={`absolute top-0 left-0 w-2 h-2 md:w-4 md:h-4 border-t-2 border-l-2 transition-colors ${item.color === "cyan" ? "border-cyan-500/50 group-hover:border-cyan-400" : "border-purple-500/50 group-hover:border-purple-400"}`}
                    />
                    <div
                      className={`absolute top-0 right-0 w-2 h-2 md:w-4 md:h-4 border-t-2 border-r-2 transition-colors ${item.color === "cyan" ? "border-cyan-500/50 group-hover:border-cyan-400" : "border-purple-500/50 group-hover:border-purple-400"}`}
                    />
                    <div
                      className={`absolute bottom-0 left-0 w-2 h-2 md:w-4 md:h-4 border-b-2 border-l-2 transition-colors ${item.color === "cyan" ? "border-cyan-500/50 group-hover:border-cyan-400" : "border-purple-500/50 group-hover:border-purple-400"}`}
                    />
                    <div
                      className={`absolute bottom-0 right-0 w-2 h-2 md:w-4 md:h-4 border-b-2 border-r-2 transition-colors ${item.color === "cyan" ? "border-cyan-500/50 group-hover:border-cyan-400" : "border-purple-500/50 group-hover:border-purple-400"}`}
                    />

                    {/* Gradient overlay */}
                    <div
                      className={`absolute inset-0 clip-path-[polygon(8px_0,100%_0,100%_calc(100%-8px),calc(100%-8px)_100%,0_100%,0_8px)] md:clip-path-[polygon(10px_0,100%_0,100%_calc(100%-10px),calc(100%-10px)_100%,0_100%,0_10px)] ${item.color === "cyan" ? "bg-linear-to-br from-cyan-900/20 to-blue-900/20" : "bg-linear-to-br from-purple-900/20 to-indigo-900/20"}`}
                    />

                    {/* Scanline effect */}
                    <div
                      className={`absolute inset-0 bg-linear-to-b from-transparent to-transparent pointer-events-none ${item.color === "cyan" ? "via-cyan-500/5" : "via-purple-500/5"}`}
                    />
                    ...
                    <div className="relative p-3 md:p-5 grid grid-cols-[auto_1fr] grid-rows-2 gap-x-2.5 gap-y-0.5 md:gap-y-1 items-center md:block md:text-center">
                      ...
                      <div className="row-span-2 flex items-center justify-center md:mb-3 md:row-auto">
                        <div
                          className={`p-1.5 md:p-2.5 rounded-lg ${item.color === "cyan" ? "bg-cyan-500/10 border border-cyan-500/30" : "bg-purple-500/10 border border-purple-500/30"}`}
                        >
                          <item.icon
                            className={`w-4 h-4 md:w-6 md:h-6 ${item.color === "cyan" ? "text-cyan-400" : "text-purple-400"}`}
                          />
                        </div>
                      </div>
                      ...
                      {/* Label */}
                      <div
                        className={`inline-block md:mb-2 px-1.5 py-0.5 md:px-3 md:py-1 text-[8px] md:text-[10px] font-mono tracking-widest text-left md:text-center ${item.color === "cyan" ? "bg-cyan-500/10 border border-cyan-400/20 text-cyan-400/70" : "bg-purple-500/10 border border-purple-400/20 text-purple-400/70"}`}
                      >
                        {item.title}
                      </div>
                      ...
                      {/* Content */}
                      <p
                        className={`text-[11px] md:text-sm font-mono font-medium text-left md:text-center leading-tight ${item.color === "cyan" ? "text-cyan-200" : "text-purple-200"} wrap-break-word break-all`}
                      >
                        {item.content}
                      </p>
                      ...
                      <div className="hidden mt-3 md:flex items-center justify-center gap-2">
                        <div
                          className={`w-1.5 h-1.5 rounded-full animate-pulse ${item.color === "cyan" ? "bg-cyan-400" : "bg-purple-400"}`}
                        />
                        <span
                          className={`text-[9px] md:text-[10px] font-mono ${item.color === "cyan" ? "text-cyan-500/50" : "text-purple-500/50"}`}
                        >
                          ACTIVE
                        </span>
                      </div>
                  ...
                  >
                    ...
                    >
                      <div className="relative bg-black/60 backdrop-blur-sm border-2 border-cyan-500/30 p-2.5 md:p-3.5 rounded-lg transition-all duration-300 group-hover:border-cyan-400/60 group-hover:shadow-[0_0_20px_rgba(34,211,238,0.4)]">
                        ...
                        {/* Gradient background on hover */}
                        <div
                          className={`absolute inset-0 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity bg-linear-to-br ${social.color}/20`}
                        />
            ...
            >
              <div className="inline-block bg-black/60 backdrop-blur-sm border border-cyan-500/20 px-2.5 py-1 md:px-4 md:py-2 font-mono text-[9px] md:text-xs text-cyan-400/50">
                <span className="text-cyan-500/70">NEED HELP?</span> Reach out
                anytime
              </div>
            </motion.div>
          </motion.div>
